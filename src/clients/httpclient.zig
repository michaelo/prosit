...
fn boolToInt(val: bool) usize {
    if (val) return 1;
    return 0;
}

/// Main API
pub fn request(allocator: std.mem.Allocator, method: HttpMethod, url: [:0]const u8, comptime params: struct {
    ...
    // insecure?
    if (cURL.curl_easy_setopt(handle, cURL.CURLOPT_SSL_VERIFYPEER, @as(c_long, @intCast(boolToInt(params.insecure)))) != cURL.CURLE_OK) {
    ...
    if (cURL.curl_easy_setopt(handle, cURL.CURLOPT_FOLLOWLOCATION, @as(c_long, @intCast(boolToInt(params.follow_redirect)))) != cURL.CURLE_OK) {
        return error.CouldNotSetFollow;
    }

    // Verbosity
    if (cURL.curl_easy_setopt(handle, cURL.CURLOPT_VERBOSE, @as(c_long, @intCast(boolToInt(params.verbose)))) != cURL.CURLE_OK) {
    ...
    // Payload
    //     if (method == .POST or method == .PUT or params.payload.len > 0) {
    //         if (cURL.curl_easy_setopt(handle, cURL.CURLOPT_POSTFIELDSIZE, params.payload.len) != cURL.CURLE_OK)
    //             return error.CouldNotSetPostDataSize;
    //         if (cURL.curl_easy_setopt(handle, cURL.CURLOPT_POSTFIELDS, params.payload.ptr) != cURL.CURLE_OK)
